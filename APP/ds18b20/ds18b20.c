...
float DS18B20_GetTemperture(void)
{
	...
	rt_thread_delay(750);                      // 等待转换完成，假设12位精度需要750ms
	//延时语句放到读写字节语句之后无效，语句放到后面，实际转换已经完成，只是在取数据
	//温度转换是由传感器内部的硬件自动完成的，而 MCU（微控制器）只需发起转换命令并在转换完成后读取结果。
	//使用delay_ms会导致温度数据整数部分读取异常
    DS18B20_Reset();
    DS18B20_Check();	 
    DS18B20_Write_Byte(0xcc);// skip rom
